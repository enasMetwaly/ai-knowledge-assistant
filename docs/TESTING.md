...
All tests are designed to run **both locally (for fast iteration) and inside Docker **.  
...
##  Manual Testing (Rate Limit Validation )
...
# Get token
TOKEN=$(curl -s -X POST http://localhost:8000/auth/login -d "username=user@example.com&password=password123" | jq -r .access_token)
